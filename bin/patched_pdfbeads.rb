...
# #2
# tesseract can output hocr that contains a positoned line of zero length
# text (e.g. "").  this blows up pdfbeads because this zero length text
# results in pdfbeads to divide by zero ... that id bad. so we are now
...
class PDFBeads::PDFBuilder
  #redefined method to remote zero length words
  old_getOCRUnits = instance_method(:getOCRUnits)
  define_method(:getOCRUnits) do |ocr_line,lbbox,fsize,charset,xscale,yscale|
    units = old_getOCRUnits.bind(self).call(ocr_line,lbbox,fsize,charset,xscale,yscale)
